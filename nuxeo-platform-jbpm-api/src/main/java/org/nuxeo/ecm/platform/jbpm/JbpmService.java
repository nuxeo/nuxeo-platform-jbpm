 ...
 * <p>
 * The methods on document assume a single document attached to a process. The variables 'documentId' and
 * 'documentRepositoryName' should exists. Security is only implemented in such a case.
 * <p>
 * Warning: The returned result of most of the methods are list issue from hibernate query. Also the object are fully
 * loaded, their dependent objects might not have been fetch. If you need dependent object, use the executeJbpmOperation
 * making sure those object are properly loaded inside the operation.
 *
...
public interface JbpmService {
     ...
     */
    List<TaskInstance> getCurrentTaskInstances(final NuxeoPrincipal currentUser, JbpmListFilter filter)
            throws NuxeoJbpmException;
    ...
    /**
     * Returns a list of task instance for this process instance. It returns all task instances, not only the tasks for
     * this principal. The principal is used as an argument to the filter.
     *
     ...
     * @throws NuxeoJbpmSecurityRuntimeException If a document is attached to the process and the transition is not
     *             allowed for user.
     */
    void endTask(Long taskInstanceId, String transition, Map<String, Serializable> taskVariables,
            Map<String, Serializable> variables, Map<String, Serializable> transientVariables, NuxeoPrincipal principal)
            throws NuxeoJbpmException;

    /**
     * Returns a list of of possible transition for this user. (If a document is attached to the process, then return
     * only the allowed transition)
     *
     ...
     *
     * @param actors A list of string that would be used as actorId to retrieve the process instances.
     * @param filter
     * @return
     * @throws NuxeoJbpmException
     */
     ...
     * @return
     */
    ProcessInstance getProcessInstance(final Long processInstanceId) throws NuxeoJbpmException;
     ...
     * @throws NuxeoJbpmException
     * @throws NuxeoJbpmSecurityException If the user is not allowed to create a process instance for this document.
     */
    ProcessInstance createProcessInstance(NuxeoPrincipal user, String processInstanceName, DocumentModel dm,
            Map<String, Serializable> variables, Map<String, Serializable> transientVariables)
            throws NuxeoJbpmException;

    /**
     * Returns a list of available Process Definition Name available for this document and user. All process definition
     * if dm is <code>null</code>. The returned process definition is always the latest.
     *
     ...
     * @throws NuxeoJbpmException
     */
    Boolean getPermission(ProcessInstance pi, JbpmSecurityPolicy.Action action, DocumentModel dm,
